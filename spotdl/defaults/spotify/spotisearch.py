...
def get_track_by_name(song_name: str) -> dict:
    """
    ### Args
...
def get_track(track_id: str):
    ...
    - None
    """
    return __track_to_metadata(sp.track(track_id))


def get_tracks(track_ids: List[str]):
    """
    ### Args
    - track_ids: `List[str]`, List of URL/URI/ID of the tracks to be found (maximum of 50)

    ### Returns
    - `List[dict]`, List of metadata of the tracks found in track_ids

    ### Errors raised
    - 'ValueError': When the list of tracks is above the maximum of 50

    ### Notes
    - None
    """
    if len(track_ids) > 50:
        raise ValueError("The get_tracks method allows a maximum of 50 tracks")
    return [__track_to_metadata(track) for track in sp.tracks(track_ids)["tracks"]]
